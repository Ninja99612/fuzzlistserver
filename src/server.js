...
app.enable("trust proxy") //ip ::1 local host
app.set("etag", false) 

  app.engine("dscbots.tk", ejs.renderFile);
  ...
  global.checkAuth = (req, res, next) => {
    ...
    res.redirect("/login");
        ...
        const Hook = new webhook.Webhook("https://discord.com/api/webhooks/954132491332091985/0Yta7zSM53qf0DfPlgogZIu4PCpwMVdLsrxnkalz3S5FgXEhqwFCi9PGL583GSmJNUyc")

  app.use(async (req, res, next) => {
    var  i = 0
    var v = 0
      if(req.ip == '34.123.7.44'){
          i++
          if(i == 2 || i >= 2){
                               const msg4 = new webhook.MessageBuilder()
    .setName('Dscbots-Tk | Tracer')
    .setDescription('IP is blocked fully now!')
    .setColor('#ff0000')
 return Hook.send(msg4)
          }else{
                 const msg2 = new webhook.MessageBuilder()
    .setName('Dscbots-Tk | Tracer')
    .setDescription('IP is blocked!')
    .setColor('#ff0000')
         setTimeout(() => {
                  Hook.send(msg2)      
              }, 20000)
...
if(v == 60 || v > 60){
    const msg3 = new webhook.MessageBuilder()
        .setName('Dscbots-Tk | Tracer')
    .setDescription('Webhook on Cooldown')
    .setColor('#ff0000')
              setTimeout(() => {
                  Hook.send(msg3)
                  v = 0
  ...
  app.get("/robots.txt", function(req, res) {
    ...
    res.send(`Sitemap: https://Disbots.dragonytdev.repl.co/sitemap.xml`);
  });
  app.get("/sitemap.xml", async function(req, res) {
    let link = "<url><loc>https://Disbots.dragonytdev.repl.co/</loc></url>";
    let botdataforxml = await botsdata.find()
    botdataforxml.forEach(bot => {
      link += "\n<url><loc>https://Disbots.dragonytdev.repl.co.xyz/bot/" + bot.botID + "</loc></url>";
    })
  ...
  app.get("/callback", passport.authenticate("discord", {
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://fuzzlist.xyz/user/${a.id}) tried to login but could not login because they were banned from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    })
    req.session.destroy(() => {
      res.json({
        login: false,
        message: "You have been banned from dscbots.",
        logout: true
      })
      req.logout();
    });
    ...
    client.users.fetch(req.user.id).then(async a => {
      client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
        ...
        dynamic: true
      })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://fuzzlist.xyz/user/${a.id}) logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
...
app.post("/admin/news", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  db.push(`news`, {
    code: createID(12),
    icon: req.body.icon,
    banner: 'https://www.dscbots.tk/img/BG.png',
    ownerID: req.user.id,
  ...
  const Hook = new webhook.Webhook(process.env.webh);
  const msg = new webhook.MessageBuilder()
    .setName('Dscbots-Tk | News')
    .setAvatar(req.body.icon)
    .setTitle(req.body.serverName)
    .setDescription(`<@${req.user.id}> Posted a News \n\nLink:\n[Look at the news](https://www.dscbots.tk/news)`)
    .setColor('#0099ff')
  Hook.send(msg);
...
app.get("/admin/premium/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.guilds.fetch(serverdata.id).then(bota => {
    client.channels.cache.get(config.server.channels.botlog).send(`<:promoted:934390850266492950> <@${serverdata.ownerID}>'s server **${bota.name}** has been promoted to **Premium**.`)
    client.users.cache.get(serverdata.ownerID).send(`<:promoted:934390850266492950> Your server named **${bota.name}** has been promoted to **Premium**.`)
  });
...
app.get("/admin/premium/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.guilds.fetch(serverdata.id).then(bota => {
    client.channels.cache.get(config.server.channels.botlog).send(`<:notcheck:934397613267099698> <@${serverdata.ownerID}>'s server named **${bota.name}**'s Premium has been removed.`)
    client.users.cache.get(serverdata.ownerID).send(`<:notcheck:934397613267099698> Your server named **${bota.name}**'s Premium has been removed.`)
  });
...
app.get("/admin/boost/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:notcheck:934397613267099698> <@${botdata.ownerID}>'s bot named **${bota.tag}**'s promotion has been removed.`)
    client.users.cache.get(botdata.ownerID).send(`<:notcheck:934397613267099698> Your bot named **${bota.tag}**'s boost has been removed.`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.get("/admin/server/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.channels.cache.get(channels.botlog).send(`<:Decline:933421764648792095> <@${botdata.ownerID}>'s server named **${botdata.name}** has been deleted by ${req.user.username}.`)
...
app.get("/admin/promote/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:notcheck:933421764648792095> <@${botdata.ownerID}>'s bot named **${bota.tag}**'s promotion has been removed.`)
    client.users.cache.get(botdata.ownerID).send(`<:notcheck:933421764648792095> Your bot named **${bota.tag}**'s promotion has been removed.`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.get("/admin/boost/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:promoted:934390850266492950> <@${botdata.ownerID}>'s bot  **${bota.tag}** has been **Boosted**.`)
    client.users.cache.get(botdata.ownerID).send(`<:promoted:934390850266492950> Your bot named **${bota.tag}** has been **Boosted**.`)
  });
  let guild = client.guilds.cache.get(config.server.id)
...
app.get("/admin/promote/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(channels.botlog).send(`<:promoted:934390850266492950> <@${botdata.ownerID}>'s bot  **${bota.tag}** has been **Promoted**.`)
    client.users.cache.get(botdata.ownerID).send(`<:promoted:934390850266492950> Your bot named **${bota.tag}** has been **Promoted**.`)
  });
  let guild = client.guilds.cache.get(config.server.id)
...
app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    ...
    client.users.cache.get(botdata.ownerID).send(`<:notcheck:934397613267099698> Your bot named **${bota.tag}** certificate application has been declined.\nReason: **${rBody['reason']}**`)
  });
  await appsdata.deleteOne({
    botID: req.params.botID
  })
...
app.use(async (req, res, next) => {
  if (req.path.includes('/admin')) {
    if (req.isAuthenticated()) {
      if (client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.yonetici)) {
        next();
      } else {
        res.redirect("/error?code=403&message=You is not competent to do this.")
      }
