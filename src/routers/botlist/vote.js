...
app.post("/bot/:botID/vote", global.checkAuth, async (req, res) => {
    ...
    await votes.findOneAndUpdate({
        bot: req.params.botID,
        username: client.users.cache.get(req.user.id).username,
        user: req.user.id
    }, {
        $set: {
            Date: Date.now(),
    ...
    client.channels.cache.get(global.config.server.channels.votes).send(`**${botdata.username}** just got **+1 Vote** from **${req.user.username}** **\`[Total Votes ${botdata.votes + 1}]\`**`)
    if(botdata.votes+1 == 100) {
    client.channels.cache.get(global.config.server.channels.votes).send(`Congrats ${botdata.ownerID}! Your bot **${botdata.username}** has reached 100 votes!!`)
    }
    if(botdata.votes+1 == 50) {
    client.channels.cache.get(global.config.server.channels.votes).send(`Congrats <@${botdata.ownerID}>! Your bot **${botdata.username}** has reached 50 votes!!`)
    }
