...
app.get("/bot/:botID", async (req, res, next) => {
    ...
    if (botdata.status != "Approved") {
        ...
        if (req.user.id == botdata.ownerID || botdata.coowners.includes(req.user.id) || req.user.id == "483357154502377473") {
            ...
            res.redirect("/error?code=404&message=To view this bot, you must be one of its owners.");
        }
    } else {
        let coowner = new Array()
        botdata.coowners.map(a => {
        ...
        */
        ...
        await client.users.fetch(botdata.ownerID).then(async(aowner) => {
           await client.users.fetch(botdata.botID).then(async(abot) => {
                    res.render("botlist/bot/bot.ejs", {
                        bot: global.Client,
                        path: req.path,
                        config: global.config,
...
app.post("/bot/:botID", global.checkAuth, async (req, res) => {
    ...
    console.log(botdata.botID + " : " + botdata.ownerID)
