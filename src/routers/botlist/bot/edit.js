...
console.log("[disbots.xyz]: Botlist/Edit router loaded.");
...
app.post("/bot/:botID/edit", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(a => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<:edit:934088957623418920> <@${req.user.id}> edited **${a.tag}**`)
