...
app.post("/admin/maintence", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(global.config.server.channels.webstatus).send(`<:NSFW:892292471931953182> Dscbots has been switched to __Maintenance__ due to **${req.body.reason}** [||@here||]`).then(a => {
        new maintenceSchema({
            server: config.server.id,
...
app.post("/admin/unmaintence", global.checkAuth, async (req, res) => {
    ...
    const bakimsonaerdikardesdisbots = new dc.MessageEmbed()
        .setAuthor("Dscbots.tk", client.user.avatarURL())
        .setThumbnail(client.user.avatarURL())
        .setColor("GREEN")
        .setDescription(`<:online:918574476969513031><:promoted:934390850266492950> Dscbots are **active** again!\n[Click to redirect website](https://www.dscbots.tk)`)
        .setFooter("Disbots © All rights reserved.");
    await client.channels.cache.get(global.config.server.channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
        a.edit(`~~ <:online:918574476969513031> Dscbots has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesdisbots)
    })
    client.channels.cache.get(global.config.server.channels.webstatus).send(".").then(b => {
        b.delete({
            timeout: 500
        })
    })
