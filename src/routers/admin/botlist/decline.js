...
app.post("/admin/decline/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.ownerID).then(sahip => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<:Decline:933421764648792095> <@${botdata.ownerID}>'s bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}** `)
        client.users.cache.get(botdata.ownerID).send(`<:Decline:933421764648792095> Your bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
    })
