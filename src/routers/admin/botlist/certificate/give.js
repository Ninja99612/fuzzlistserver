...
app.get("/admin/certificate/give/:botID", global.checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(global.config.server.channels.botlog).send(`<:promoted:934390850266492950> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been granted a certificate.`)
    client.users.cache.get(botdata.ownerID).send(`<:promoted:934390850266492950> Your bot named **${bota.tag}** has been certified.`)
  });
