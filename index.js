...
client.on('presenceUpdate', async(oldPresence, newPresence) => 
{
   ...
   var botdata = await botsdata.findOne({ botID: newPresence.userID });
    ...
    if(newPresence.guild.id == "800011230596956161")
    {
...
if(!uptimerate)
        ...
        if(breh === newPresence.userID)
      {
       ...
       client.channels.cache.get("933405832400498708").send(`<@${newPresence.userID}> is Offline And Uptime Rate - ${uptimerate}%`) 
   ...
   if(!uptimerate)
      {
       ...
       client.channels.cache.get("933405832400498708").send(`<@${newPresence.userID}> is Online And Uptime Rate - ${uptimerate}% And was Offline for ${hour}h ${minutes}m ${seconds}s`) 
       db.set(`timefr_${newPresence.userID}`, Date.now())
    }
    }
...
client.on('voiceStateUpdate', async(oldState, newState) => {
  ...
  if (!oldState.channel) {
    ...
    if (dateDiff) {
      ...
      var find = await profiledata.findOne({ userID: newState.member.id })
        ...
        await new profiledata({
         ...
         client.channels.cache.get('933424507115434044').send(`Hey <@${newState.member.id}>, We've noticed you had no profile so we couldn't add any **Dscbots Coins** to your profile please try again.`)
      return
      }
    ...
    client.channels.cache.get('933424507115434044').send(`Hey <@${newState.member.id}>, Thankyou for being active! You have got some **Dscbots Coins** for being active!`)
    }
  }
})
...
client.on('guildMemberRemove', async member => {
    ...
    client.guilds.fetch(config.server.id).then(bota => {
      client.channels.cache.get(config.server.channels.botlog).send(`<:Decline:933421764648792095> <@${data.ownerID}>'s bot **${bota.username}** has been kicked from server and deleted from website \nReason: Owner left server`)
    });
    ...
    client.guilds.fetch(config.server.id).then(bota => {
      client.channels.cache.get(config.server.channels.botlog).send(`<:Decline:933421764648792095> <@${serverfind.ownerID}>'s server **${serverfind.name}** has been deleted from website \nReason: Owner left server`)
    });
...
client.on('ready',async () => {
    ...
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'FUZZLIST.XYZ | '+bots.length+' bots' }, status: "dnd" });
});
